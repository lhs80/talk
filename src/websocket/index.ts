...
type send<T> = (msg: WsMessage<T>) => void;
...
class SocketCore extends TimedTask {
  ...
  constructor() {
    ...
    this.options = {
      ...
      heartMsg: SocketCore.heartbeatMessage(), // 心跳信息
      ...
      renewMsg: SocketCore.renewMessage(),
  ...
  create(ops: Ioptions): void {
    if (this.wsInstance === null || this.wsInstance === undefined || this.wsInstance.readyState !== WebSocket.OPEN) {
      ...
      this.wsInstance = new WebSocket(this.options.url, [this.options.token as string]);
      this.onopen();
      this.onclose();
      this.onerror();
      this.onmessage();
  ...
  private onclose(): void {
    this.wsInstance.onclose = (event: any) => {
      if (!this.options.isDestroy) {
        console.log(
          "%s 正常注销后 WebSocket 关闭, code:%d-%s",
          new Date().toLocaleString(),
          event.code,
          websocketErrorCode.get(event.code)
        );
      }
      if (event.code === 1000) {
        console.log(
          "%s WebSocket 正常关闭, code:%d-%s",
          new Date().toLocaleString(),
          event.code,
          websocketErrorCode.get(event.code)
        );
   ...
   */
  onmessage(): void {
    this.wsInstance.onmessage = (event: MessageEvent<string>) => {
      ...
      if (wsMessage.type === "ERROR") {
        ...
        // 可以发送心跳时,恢复重连次数
        // this.reconnectCount = this.options.reconnectCount as number;
        this.wsInstance.send(JSON.stringify(SocketCore.heartbeatMessage()));
        // 开启心跳发送
        ...
        if (wsMessage.receipt !== 0) {
          this.send(SocketCore.receiptMessage(wsMessage))
        }
        // 抛出消息到消息回执函数
        if (typeof this.options.messageCb === "function") {
          this.options.messageCb(wsMessage);
  ...
  onReconnect(): void {
    if (!this.options.isDestroy) {
      if (this.reconnectCount > 0 || this.reconnectCount === -1) {
        console.error("%s websocket 重连[%d-%d]",
          new Date().toLocaleString(),
          this.reconnectCount,
        ...
        console.error("%s 重连次数已达到最大,结束重连[%d-%d]",
          new Date().toLocaleString(),
          this.reconnectCount,
          this.options.reconnectCount);
        clearTimeout(this.reconnectTimer);
  ...
  destroy(closeCode?: number, closeMessage?: string): void {
    console.log("%s 销毁 websocket 连接,销毁代码:%d,附带信息:%s",
      new Date().toLocaleString(),
      closeCode,
      closeMessage
    );
    ...
    this.wsInstance.close(closeCode, closeMessage);
  }
...
class UicpSocket {
  ...
  /**
   * private 阻止外部通过 new 创建实例
   */
  private constructor() {
  }

  private static instance: UicpSocket | null

  /**
   * 获取 UicpSocket 实例,保证对象单例
   * @returns UicpSocket 实例
   */
  static getInstance(): UicpSocket {
    if (UicpSocket.instance === null || UicpSocket.instance === undefined) {
      UicpSocket.instance = new UicpSocket();
    }
    return UicpSocket.instance;
  }
   ...
   * @param options 连接参数
   */
  public createSocket(options: Ioptions): void {
    if (!this.socketCore) {
      this.socketCore = new SocketCore();
    }
    this.socketCore.create(options);
  }
   ...
   * @param cb 回调函数
   */
  public setOnMessage(cb: MessageCallback<WsMessage<any>>): void {
    this.socketCore.options.messageCb = cb
  }
   ...
   * @param message 消息
   */
  public sendMessage<T>(message: WsMessage<T>): void {
  ...
  public renew(): void {
    ...
    this.socketCore.send(this.socketCore.options.renewMsg as WsMessage<null>);
  }

  /**
   * 开启自动续约,(4次心跳续约1次)
   */
  public autoRenew(): void {
    console.log("%s 开启自动续约", new Date().toLocaleString());
    this.socketCore.options.autoRenew = true;
    this.socketCore.reStartRenew(() => {
      this.socketCore.send(this.socketCore.options.renewMsg as WsMessage<null>);
    }, this.socketCore.options.hearInterval as number * 4);
  }
  ...
  public stopAutoRenew(): void {
    ...
    this.socketCore.options.autoRenew = false;
    this.socketCore.stopRenew();
  }
  ...
  public destroySocket(): void {
    if (this.socketCore) {
      this.socketCore.destroy(1000, websocketErrorCode.get(1000));
    }
  }
}