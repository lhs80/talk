<template>
  ...
  <div class="order-wrapper">
    <add-meeting-form ref="meetingForm" :data="state.formData.meetingInfo" page="again"/>
    <add-person-form ref="personForm" :isDisable="true" :data="state.formData.personInfo" page="again"/>
...
<script setup lang="ts">
  ...
  import {createMeetingApi, getMeetingInfoApi} from "@/api/modules/mcs";
  import {noteCreateApi} from "@/api/modules/ess";
  import {searchCaseByNameApi, addCaseAndPersonApi} from "@/api/modules/cms";
  ...
  const submit = async () => {
    ...
    if (personFormValid && meetingFormValid) {
      state.isSubmit = true;
      let {caseId, collegialPerson, assistPerson, recordPerson, meetingName, uploadFileDate, caseName, meetingCode, meetingEndTime, meetingStartTime, id, ...otherParams} = meetingForm.value.meetingInfo;
      ...
      let params = {
        ...otherParams,
        materialUploadStartDate: uploadFileDate.length && uploadFileDate[0] ? uploadFileDate[0] : null,
        materialUploadStopDate: uploadFileDate.length && uploadFileDate[1] ? uploadFileDate[1] : null,
        meetingName,
        caseId,
      ...
      createMeetingApi(params).then((response) => {
        if (response.success) {
          ...
          //4、创建笔录
          createNote(response.data.meetingId);
          close();
        } else {
          state.isSubmit = false;
        }
  ...
  const createNote = async (orderId) => {
    let noteParams = {
      ...
      oldOrderId: state.formData.meetingInfo.id,
      orderId
    };
    let noteResponse = await noteCreateApi(noteParams);
    close();
  };

  //提交会议信息前，判断案件是否存在
  const caseIsExist = async (meetingName) => {
    const response = await searchCaseByNameApi(meetingName);
    if (response.success) {
  ...
  const getMeetingInfo = async () => {
    if (state.meetingId) {
      ...
      if (response.success) {
        let {interiorInviteMembers, externalInviteMembers, interiorJoinMembers, externalJoinMembers, noteFileId, password, recordFileId, materialUploadStartDate, materialUploadStopDate, ...otherParams} = response.data;
...
<style lang="scss">
  .order-wrapper {
    width: 100%;
    height: 100%;
    background-image: url("../../assets/img/bg-order.png") white no-repeat bottom left / 100% 30%;
